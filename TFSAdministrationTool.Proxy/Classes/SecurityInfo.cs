...
namespace TFSAdministrationTool.Proxy.Common
{
    public class SecurityInfo
    {
        ...
        public List<SecurityGroup> GetGroupBySidOrLoginName(string sid, string loginname)
        {
            ...
            foreach (SecurityGroup sGroup in m_SecuritySettings)
            {
                ...
                {
                    if (String.Compare(sid, u.Sid, true) == 0 || String.Compare(loginname, u.LoginName, true) == 0)
                    {
                        groups.Add(sGroup);
                        break;
                    }
    ...
    public class SecurityGroup
    {
        ...
        public void AddUser(string sid, string username, string displayname, string loginname)
        {
            m_Users.Add(new User() { Sid = sid, UserName = username, DisplayName = displayname, LoginName = loginname });
        }

        public void AddUser(string sid, string username, string displayname, string email, Microsoft.TeamFoundation.Server.IdentityType identityType, string loginname)
        {
            m_Users.Add(new User() { Sid = sid, UserName = username, DisplayName = displayname, Email = email, IdentityType = identityType, LoginName = loginname });
        }
